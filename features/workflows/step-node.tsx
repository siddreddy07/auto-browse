...
export function StepNode({ data, selected }: NodeProps) {
  const nodeType =
    data.type || (data.definition as { type?: string } | undefined)?.type
    ...
    >
      <div className="flex items-center gap-2 border-b px-3 py-2">
        <div
          ...
          style={{
            backgroundColor: definition.accent + "20",
        ...
        <span className="truncate text-sm font-medium">
          {String(data.displayLabel || definition.label)}
        </span>
        {definition.premium && (
          <Crown className="ml-1 size-3 shrink-0 text-amber-500" />
        )}
      ...
      {status === "failed" && Boolean(data.error) && (
        <div className="px-3 py-2">
          <p className="line-clamp-3 text-xs text-red-500">
            {String(data.error)}
          </p>
        </div>
      )}
      ...
      {!isStart && (
        <Handle
          ...
          className="!h-4 !w-2 !rounded-sm !border-2 !border-background"
        />
      )}
      <Handle
        type="source"
...
function StatusIndicator({ status }: { status?: NodeStatus }) {
  if (status === "running")
    return (
      <Loader2 className="ml-auto size-4 shrink-0 animate-spin text-blue-500" />
    )
  if (status === "done")
    return <CheckCircle2 className="ml-auto size-4 shrink-0 text-emerald-500" />
  if (status === "failed")
    return <XCircle className="ml-auto size-4 shrink-0 text-red-500" />
  if (status === "pending")
    return (
      <Timer className="ml-auto size-4 shrink-0 text-muted-foreground/60" />
    )
  return null
}
